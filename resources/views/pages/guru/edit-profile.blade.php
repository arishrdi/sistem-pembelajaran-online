...
<div class="flex flex-col lg:flex-row">
  <div class="card border w-5/6 lg:w-[30rem] mx-auto">
    <form action="{{route('update-guru', ['guru' => $user->nip])}}" method="POST" class="w-[30rem] mx-auto py-4 px-6">
      @method('PUT')
      @csrf
      ...
      <input type="text" name="nip" class="input input-secondary w-full" value="{{$user->nip}}" readonly>
      @error('nip')
      <label class="label">
      ...
      <input type="text" name="nama_guru" class="input input-secondary w-full" value="{{$user->nama_guru}}">
      @error('nama_guru')
      <label class="label">
        <span class="label-text-alt text-error">{{$message}}</span>
      </label>
      ...
      <input type="text" name="gelar" class="input input-secondary w-full" value="{{$user->gelar}}">
      @error('gelar')
      <label class="label">
        <span class="label-text-alt text-error">{{$message}}</span>
      </label>
      ...
      <div class="card-actions justify-end my-3">
        <a href="" onclick="history.back()"  type="submit" class="btn btn-error btn-outline">Kembali</a>
